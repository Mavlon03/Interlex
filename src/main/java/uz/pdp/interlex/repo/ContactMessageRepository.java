...
public interface ContactMessageRepository extends JpaRepository<ContactMessage, Long> {
    ...
    @Query("SELECT COUNT(cm) FROM ContactMessage cm WHERE cm.status = uz.pdp.interlex.entity.ContactMessage.MessageStatus.CLOSED")
    Long countSuccessfulCases();

    @Query("SELECT COUNT(cm) FROM ContactMessage cm WHERE cm.createdAt >= :cutoffDate")
    Long countCasesSince(@Param("cutoffDate") LocalDateTime cutoffDate);

    @Query("SELECT cm.serviceType, COUNT(cm) as caseCount " +
            "FROM ContactMessage cm " +
            "GROUP BY cm.serviceType " +
            "ORDER BY caseCount DESC")
    List<Object[]> countCasesByServiceType();
}
